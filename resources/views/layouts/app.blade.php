...
<body class="font-sans antialiased">
    @php
        $navLinks = [
            ['label' => 'Beranda', 'route' => 'dashboard', 'icon' => '<path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" />'],
            ['label' => 'Produk', 'route' => 'produk.index', 'icon' => '<path d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 0 000-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 3H6.28l-.31-1.243A1 1 0 005 1H3zM16 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM6.5 18a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" />'],
            ['label' => 'Promo', 'route' => 'promo.index', 'icon' => '<path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" /><path fill-rule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm9.707 5.707a1 1 0 00-1.414-1.414L9 12.586l-1.293-1.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />'],
            [
                'label' => 'Gaji', // Nama diperpendek agar muat di HP
                'route' => 'gaji.index', // Pastikan route ini ada di web.php
                'icon' => '<path fill-rule="evenodd" d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" />'
            ]
        ];
    @endphp
    ...
    <div class="flex min-h-screen bg-gray-100" x-data="{ sidebarOpen: false, mobileMenuOpen: false }">
            ...
            <div>
                <div class="mb-10 mt-2 flex items-center justify-center">
                    <template x-if="sidebarOpen">
                        <img src="{{ asset('asset/Logo 2.png') }}" alt="Logo" class="h-10 w-auto transition duration-300" />
                    </template>
                    <template x-if="!sidebarOpen">
                ...
                <nav class="space-y-2">
                    @foreach ($navLinks as $link)
                        <a href="{{ route($link['route']) }}"
                            ...
                            <span class="whitespace-nowrap transition-opacity duration-300" x-show="sidebarOpen">{{ $link['label'] }}</span>
                        </a>
                    @endforeach
                </nav>
                ...
                <div x-data="{ open: false }" class="text-white mt-4">
                    <button @click="open = !open"
                        ...
                        <svg x-show="sidebarOpen" xmlns="http://www.w3.org/2000/svg" :class="open ? 'rotate-180' : ''"
                            class="h-4 w-4 transform transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex-1 flex flex-col pb-16 lg:pb-0">
            <header class="bg-gradient-to-r from-white via-[#f7e0c4]/20 to-white backdrop-blur-md shadow-lg sticky top-0 z-10 border-b-2 border-[#cc9966]/30">
                <div class="px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex justify-between items-center">
                    <div class="flex items-center gap-2 sm:gap-3">
                        ...
                        <h2 class="text-lg sm:text-2xl font-bold bg-gradient-to-r from-[#7a3939] to-[#cc9966] bg-clip-text text-transparent">Ringkasan Beranda</h2>
                    </div>

                    <div class="flex items-center space-x-3 sm:space-x-6">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 text-gray-500 hover:text-[#cc9966] cursor-pointer transition duration-150" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>

                        <div class="relative cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 text-gray-500 hover:text-[#cc9966] transition duration-150" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <!-- Mobile menu -->
            <div x-show="mobileMenuOpen" x-transition @click.away="mobileMenuOpen = false" class="lg:hidden bg-white shadow-lg border-b-2 border-[#cc9966]/30 sticky top-[60px] z-10">
                <nav class="p-4 space-y-2">
                    @foreach ($navLinks as $link)
                        <a href="{{ route($link['route']) }}" class="flex items-center p-3 rounded-lg transition duration-300 {{ request()->routeIs($link['route']) ? 'bg-[#cc9966] text-white font-semibold' : 'text-gray-700 hover:bg-gray-100' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">{!! $link['icon'] !!}</svg>
                            {{ $link['label'] }}
                        </a>
                    @endforeach
        ...
        <nav class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t-2 border-[#cc9966]/30 shadow-2xl z-30">
            <div class="grid grid-cols-3 h-16">
                @foreach ($navLinks as $link)
                    <a href="{{ route($link['route']) }}" class="flex flex-col items-center justify-center transition duration-200 {{ request()->routeIs($link['route']) ? 'text-[#cc9966] font-semibold' : 'text-gray-500 hover:text-[#7a3939]' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 20 20" fill="currentColor">{!! $link['icon'] !!}</svg>
