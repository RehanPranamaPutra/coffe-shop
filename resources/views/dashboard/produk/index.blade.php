...
@section('content')
        ...
        <div class="fixed top-5 right-5 z-[60] w-80 animate-bounce-short">
            @if ($errors->any())
                <div class="bg-red-500 text-white p-4 rounded-2xl shadow-2xl mb-3 flex items-start gap-3">
                    ...
                    <ul class="text-xs">@foreach ($errors->all() as $error) <li>{{ $error }}</li> @endforeach</ul>
                </div>
            @endif
            @if (session('success'))
    ...
    <div class="container mx-auto px-4" x-data="{ openAdd: false, openEdit: false, editData: {} }">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            @forelse ($menus as $menu)
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden flex flex-col group hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 relative">

                    <!-- IMAGE BOX (Fixed Ratio to show full context) -->
                    <div class="relative h-64 overflow-hidden bg-gray-100">
                        ...
                        <div class="absolute top-4 right-4">
                            <span class="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg {{ $menu->status == 'Tersedia' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white' }}">
                                {{ $menu->status }}
                            </span>
                        </div>
                        ...
                        <div class="absolute bottom-4 left-4">
                            <span class="bg-white/90 backdrop-blur text-[#7a3939] px-3 py-1 rounded-xl text-[10px] font-bold shadow-sm border border-white/20 uppercase tracking-tighter">
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="text-xl font-black text-gray-800 leading-tight group-hover:text-[#7a3939] transition-colors uppercase">
                                {{ $menu->nama_menu }}
                            </h3>
                        </div>
                        ...
                        <div class="mt-auto">
                            <div class="flex items-center gap-2 mb-4 bg-[#fdf8f3] p-2 rounded-2xl w-fit">
                                <svg class="w-4 h-4 text-[#cc9966]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('produk.varian.index', $menu->id) }}"
                                   class="flex-1 bg-[#cc9966] text-white py-3 rounded-2xl text-center text-xs font-bold hover:bg-[#b38659] transition-all shadow-md flex items-center justify-center gap-2">
                                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m.599-1c.538-.128 1.051-.36 1.511-.679M12 16c-1.11 0-2.08-.402-2.599-1M12 16V15" /></svg>
                                    Atur Varian
                                </a>

                                <!-- Edit -->
                                <button @click="editData = @js($menu); openEdit = true"
            ...
            @empty
                <div class="col-span-full py-20 flex flex-col items-center justify-center bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
                    <img src="{{ asset('asset/empty_menu.png') }}" class="w-40 opacity-20 grayscale mb-4">
                    <p class="text-gray-400 font-bold uppercase tracking-widest text-sm text-center">Belum ada menu yang terdaftar.<br><span class="font-normal normal-case italic">Silakan klik tombol tambah untuk memulai.</span></p>
                </div>
            @endforelse
        </div>
        ...
        <div x-show="openAdd" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div x-show="openAdd" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" @click="openAdd = false" class="absolute inset-0 bg-black/60 backdrop-blur-sm"></div>

            <div x-show="openAdd" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-8 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
                ...
                <div class="flex justify-between items-center mb-8">
                    ...
                    <button @click="openAdd = false" class="text-gray-400 hover:text-red-500 transition-colors">
                ...
                <form action="{{ route('produk.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-900">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="nama_menu" required class="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-[#cc9966] transition-all placeholder-gray-300 font-bold" placeholder="Contoh: Americano Gula Aren">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Gambar Produk</label>
                            <input type="file" name="gambar" required class="w-full text-xs text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-[#cc9966] file:text-white hover:file:bg-[#7a3939] transition-all cursor-pointer">
                        </div>
                    ...
                    <div class="flex gap-4 pt-4">
                        <button type="button" @click="openAdd = false" class="flex-1 py-4 text-gray-400 font-bold hover:text-gray-600 transition-colors uppercase tracking-widest text-xs">Batal</button>
        ...
        <div x-show="openEdit" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div x-show="openEdit" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" @click="openEdit = false" class="absolute inset-0 bg-black/60 backdrop-blur-sm"></div>

            <div x-show="openEdit" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
                ...
                <form :action="'{{ url('produk') }}/' + editData.id" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-900">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="file" name="gambar" class="w-full text-xs text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-blue-500 file:text-white transition-all cursor-pointer">
                        </div>
                        <div class="md:col-span-2">
