    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-[#7a3939] to-[#cc9966] p-6">
                <h3 class="text-2xl font-bold text-white">Buat Promo Baru</h3>
                <p class="text-white/80 text-sm mt-1">Tentukan diskon dan periode berlakunya</p>
            </div>

            {{-- Form Promo --}}
            <form action="{{ route('promo.store') }}" method="POST" class="p-8 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="jenis_promo" id="jenis_promo" required
                            ...
                            <option value="persen" {{ old('jenis_promo') == 'persen' ? 'selected' : '' }}>Persentase (%)</option>
                            <option value="nominal" {{ old('jenis_promo') == 'nominal' ? 'selected' : '' }}>Nominal (Rp)</option>
                        </select>
                        @error('jenis_promo')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
