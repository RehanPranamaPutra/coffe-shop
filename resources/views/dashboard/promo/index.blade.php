...
@section('content')
    <div class="space-y-4 sm:space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {{-- Total Promo --}}
            <div class="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-lg sm:rounded-xl p-4 sm:p-6 text-white shadow-lg">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div class="flex-1">
                        <p class="text-indigo-100 text-xs sm:text-sm font-medium">Total Promo</p>
                        <h3 class="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2">{{ $promos->count() }}</h3>
                    </div>
            ...
            <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg sm:rounded-xl p-4 sm:p-6 text-white shadow-lg">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div class="flex-1">
                        <p class="text-green-100 text-xs sm:text-sm font-medium">Promo Aktif</p>
                        <h3 class="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2">{{ $promoAktif }}</h3>
                    </div>
                    <div class="bg-white/20 p-2 sm:p-3 rounded-lg w-fit">
                        <svg class="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg sm:rounded-xl p-4 sm:p-6 text-white shadow-lg">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div class="flex-1">
                        <p class="text-yellow-100 text-xs sm:text-sm font-medium">Akan Datang</p>
                        <h3 class="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2">{{ $promoAkanDatang }}</h3>
                    </div>
                    <div class="bg-white/20 p-2 sm:p-3 rounded-lg w-fit">
                        <svg class="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-lg sm:rounded-xl p-4 sm:p-6 text-white shadow-lg">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div class="flex-1">
                        <p class="text-red-100 text-xs sm:text-sm font-medium">Kadaluarsa</p>
                        <h3 class="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2">{{ $promoKadaluarsa }}</h3>
                    </div>
                    <div class="bg-white/20 p-2 sm:p-3 rounded-lg w-fit">
                        <svg class="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-[#7a3939] to-[#cc9966] p-4 sm:p-6">
                <div class="flex flex-col gap-3 sm:gap-4">
                    <div>
                        <h3 class="text-lg sm:text-xl font-bold text-white">Daftar Promo Aktif</h3>
                        <p class="text-white/80 text-xs sm:text-sm mt-1">Kelola semua diskon dan promosi</p>
                    </div>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b-2 border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Diskon</th>
                            ...
                            <tr class="hover:bg-gray-50 transition duration-150">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $statusClass }}">
                                        <span class="w-2 h-2 {{ $dotClass }} rounded-full"></span>
                                        {{ $statusText }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
            ...
            {{-- Mobile Card View --}}
            <div class="md:hidden divide-y divide-gray-200" id="promoCards">
                @forelse($promos as $promo)
                    @php
                        $isAktif = $promo->tanggal_mulai <= $now && $promo->tanggal_selesai >= $now;
                        $isAkanDatang = $promo->tanggal_mulai > $now;
                    ...
                    <div class="p-4 hover:bg-gray-50 transition duration-150">
                        <div class="space-y-3">
                            {{-- Header: Produk & Status --}}
                            <div class="flex items-start justify-between gap-2">
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-gray-900 text-sm sm:text-base line-clamp-1">
                                        {{ $promo->menu->nama_menu ?? 'Produk Dihapus' }}
                                    </h4>
                                    <p class="text-xs text-gray-500">(ID: {{ $promo->menu_id }})</p>
                                </div>
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold {{ $statusClass }} whitespace-nowrap flex-shrink-0">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="flex-1">
                                    ...
                                    <p class="font-bold text-xl sm:text-2xl text-[#7a3939]">{{ $diskonDisplay }}</p>
                                </div>
                                <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $jenisClass }}">
                                    {{ ucfirst($promo->jenis_promo) }}
                                </span>
                            </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-2.5">
                                <p class="text-xs text-gray-500 mb-1">Periode Promo</p>
                ...
                @empty
                    <div class="p-8 text-center">
                        ...
                        <p class="text-gray-500 font-semibold text-sm">Belum ada promo yang dibuat</p>
                        <p class="text-gray-400 text-xs mt-1">Buat diskon menarik untuk meningkatkan penjualan!</p>
                        <a href="{{ route('promo.create') }}"
    ...
    <script>
        document.getElementById('searchInput').addEventListener('keyup', function() {
            ...
            const tableRows = document.querySelectorAll('#promoTable tr');
            ...
            const cards = document.querySelectorAll('#promoCards > div');
            cards.forEach(card => {
